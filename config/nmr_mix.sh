...
# Step 1 Conformation Search using confab, and xtb
cd $workDir
if [ -f step1.xyz ] && [ "$(cat step1.xyz)" ]; then
	...
	do
		mv step1/$file tmp.xyz
		xtb tmp.xyz --input $root/config/md1.inp --omd --gfn 0 >> $workDir/logs/step1.log
		cat xtb.trj >> $workDir/step1.xyz
	done
fi
