...
workDir=$(pwd)/$dirname 
echo "$(date +%Y-%m-%d\ %H:%M:%S): start $dirname"
if [ -d $dirname ]; then
...
cd molclus
chmod +x molclus isostat xyz2QC mol2_xyz 
...
if  [ ! "$(cat step1.xyz)" ]; then
    ...
    cp isomers.xyz $workDir/step21.xyz
...
if  [ ! "$(cat step2.xyz)" ]; then
    ...
    cp isomers.xyz $workDir/step31.xyz
...
if  [ ! "$(cat step3.xyz)" ]; then
	...
	nums=$(awk '/DE/' $workDir/logs/step3_isostat.log |awk '$11<2' |wc -l)
    ...
    cp isomers.xyz $workDir/step41.xyz
